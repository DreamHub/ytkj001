...
public class CheckTreeSelectionModel extends DefaultTreeSelectionModel {
	...
	public void addPathsByNodes(List selectedNodes) {
		...
		for (int i = 0; i < num; i++) {
			DefaultMutableTreeTableNode node = (DefaultMutableTreeTableNode) selectedNodes
					.get(i);
	...
	public Boolean[] getselectedindex(TreePath[] path) {
		Boolean[] bflag = new Boolean[path.length];
		for (int i = 0; i < path.length; i++) {
			bflag[i] = isPathSelected(path[i]);
		}
		return bflag;
	}
